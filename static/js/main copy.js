...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateLayerVisibility() {
        ...
        // Função para mover camadas entre allLayers e hidingLayers
        function toggleLayer(layerId, checkbox, layerObject) {
            if (checkbox.checked && hidingLayers[layerId]) {
                // Mover a camada de hidingLayers para allLayers
                allLayers[layerId] = hidingLayers[layerId];
                hidingLayers[layerId] = null;
            } else if (!checkbox.checked && allLayers[layerId]) {
                // Mover a camada de allLayers para hidingLayers
                hidingLayers[layerId] = allLayers[layerId];
                allLayers[layerId] = null;
            }
    
            // Se a camada estiver visível, adiciona-a a layersToDisplay
            if (allLayers[layerId]) {
                layersToDisplay.push(allLayers[layerId]);
            }
        }
    
        // Aplicando a lógica de ativar/desativar camadas
        toggleLayer('coverage-area', coverageAreaCheckbox, allLayers['coverage-area']);
        toggleLayer('stops', stopsCheckbox, allLayers['stops']);
        toggleLayer('population', populationCheckbox, allLayers['population']);
        toggleLayer('clusters', clustersCheckbox, allLayers['clusters']);
        ...
        // Logs para verificar o estado atual
        console.log("Camadas visíveis:", Object.keys(allLayers).filter(id => allLayers[id]));
        console.log("Camadas invisíveis:", Object.keys(hidingLayers).filter(id => hidingLayers[id]));
        console.log("Camadas exibidas no mapa:", layersToDisplay.map(layer => layer.id));
    }
